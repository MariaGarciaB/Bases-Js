...
function elevar(num, exponent) {
  ...
  let elevandoPorExponente = Math.pow(num, exponent);
  return elevandoPorExponente;
}

//16. Redondear un número.
function redondearNumero(num) {
  // 🎯 Devolvé el valor de "num" redondeado al entero más próximo.
  let redondeando = Math.round(num);
  return redondeando;
}
...
function redondearHaciaArriba(num) {
  ...
  let redondeandoArriba = Math.ceil(num);
  return redondeandoArriba;
}
...
function areaDelRectangulo(alto, ancho) {
  ...
  // Área de rectángulo es = largo(alto) x ancho.
  let areaRectangulo = alto * ancho + " cm cuadrados";
...
function perimetroDelCuadrado(lado) {
  ...
  // Perímetro cuadrado: es la suma de sus 4 lados.
  let perimetroCuadrado = lado * 4 + " cm";
...
function areaDelTriangulo(base, altura) {
  ...
  // Área de un triángulo = base x altura, sobre 2.
...
function deEuroAdolar(euro) {
  ...
  let conversionAdolar = euro * 1.2 + " dolares";
